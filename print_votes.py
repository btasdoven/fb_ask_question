...
for date, vote in tom_games:
	if vote >= 6:
		message += u"Yarin(%s) %d kisi oynayalim demis. Oynayalim diyen herkes gelecek degil mi?\n\n" % (date, vote)
	else:
		message += u"Yarin(%s) %d kisi oynayalim dedigi icin yeterli sayiya ulasamadik.\n\n" % (date, vote)

message += u"Su anlik bu haftanin oylama sonuclari soyle:\n\n"	
